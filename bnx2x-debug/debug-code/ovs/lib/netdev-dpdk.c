...
free_dpdk_buf(struct dp_packet *p)
{
    ...
    zf_enter("rte_pktmbuf_free");
    rte_pktmbuf_free(pkt);
    zf_out("rte_pktmbuf_free","");
}
...
{
    zf_enter("dpdk_eth_dev_queue_setup");
    int diag = 0;
    int i;
    ...
    while (n_rxq && n_txq) {
        ...
        zf_out("rte_eth_tx_queue_setup * n","");
        ...
        zf_enter("rte_eth_rx_queue_setup * n");
        for (i = 0; i < n_rxq; i++) {
        ...
        zf_out("rte_eth_rx_queue_setup * n","");
        ...
        zf_out("dpdk_eth_dev_queue_setup","");
        return 0;
    }

    zf_out("dpdk_eth_dev_queue_setup","1");
    return diag;
}
...
dpdk_eth_dev_init(struct netdev_dpdk *dev) OVS_REQUIRES(dpdk_mutex)
{
    ...
    if (!rte_eth_dev_is_valid_port(dev->port_id)) {
        zf_enter("zf_info:rte_eth_dev_is_valid_port");
        return ENODEV;
    }

    zf_enter("rte_eth_dev_info_get");
    rte_eth_dev_info_get(dev->port_id, &info);
    zf_out("rte_eth_dev_info_get","");
    ...
    zf_enter("rte_eth_dev_start");
    diag = rte_eth_dev_start(dev->port_id);
    zf_out("rte_eth_dev_start","");
    
    if (diag) {
        zf_out("rte_eth_dev_start","1");
        VLOG_ERR("Interface %s start error: %s", dev->up.name,
                 rte_strerror(-diag));
        return -diag;
    }

    zf_enter("rte_eth_promiscuous_enable");
    rte_eth_promiscuous_enable(dev->port_id);
    zf_out("rte_eth_promiscuous_enable","");
    zf_enter("rte_eth_allmulticast_enable");
    rte_eth_allmulticast_enable(dev->port_id);
    zf_out("rte_eth_allmulticast_enable","");

    memset(&eth_addr, 0x0, sizeof(eth_addr));
    zf_enter("rte_eth_macaddr_get");
    rte_eth_macaddr_get(dev->port_id, &eth_addr);
    zf_out("rte_eth_macaddr_get","");
    VLOG_INFO_RL(&rl, "Port %d: "ETH_ADDR_FMT"",
                    dev->port_id, ETH_ADDR_BYTES_ARGS(eth_addr.addr_bytes));

    memcpy(dev->hwaddr.ea, eth_addr.addr_bytes, ETH_ADDR_LEN);;
    zf_enter("rte_eth_link_get_nowait");
    rte_eth_link_get_nowait(dev->port_id, &dev->link);
    zf_out("rte_eth_link_get_nowait","");

    zf_enter("rte_mempool_get_priv");
    mbp_priv = rte_mempool_get_priv(dev->dpdk_mp->mp);
    zf_out("rte_mempool_get_priv","");
    dev->buf_size = mbp_priv->mbuf_data_room_size - RTE_PKTMBUF_HEADROOM;

    dev->flags = NETDEV_UP | NETDEV_PROMISC;

    /* Get the Flow control configuration for DPDK-ETH */
    zf_enter("rte_eth_dev_flow_ctrl_get");
    diag = rte_eth_dev_flow_ctrl_get(dev->port_id, &dev->fc_conf);
    zf_out("rte_eth_dev_flow_ctrl_get","");
    if (diag) {
    zf_out("rte_eth_dev_flow_ctrl_get","1");
        VLOG_DBG("cannot get flow control parameters on port=%d, err=%d",
                 dev->port_id, diag);
    }

    zf_out("dpdk_eth_dev_init","");
    return 0;
}
...
{
    zf_enter("zf_info:dpdk_ring_create");
    ...
    zf_enter("rte_eth_dev_count");
    ivshmem->eth_port_id = rte_eth_dev_count() - 1;
    zf_out("rte_eth_dev_count","");
...
dpdk_init__(const struct smap *ovs_other_config)
{
    ...
    zf_enter("rte_eal_init");
    result = rte_eal_init(argc, argv);
    zf_out("rte_eal_init","");
    
    
    if (result < 0) {
        zf_out("rte_eal_init","1");
        ovs_abort(result, "Cannot init EAL");
    }

    /* Set the main thread affinity back to pre rte_eal_init() value */
    if (auto_determine && !err) {
        zf_enter("pthread_setaffinity_np");
        err = pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t),
                                     &cpuset);
        zf_out("pthread_setaffinity_np","");
        if (err) {
            zf_out("pthread_setaffinity_np","1");
    ...
    zf_enter("rte_memzone_dump");
    rte_memzone_dump(stdout);
    zf_out("rte_memzone_dump","");
    rte_eal_init_ret = 0;
    ...
    zf_enter("rte_pdump_init");
    err = rte_pdump_init(ovs_rundir());
    zf_out("rte_pdump_init","");
    if (err) {
        zf_out("rte_pdump_init","1");
        VLOG_INFO("Error initialising DPDK pdump");
        zf_enter("rte_pdump_uninit");
        rte_pdump_uninit();
        zf_out("rte_pdump_uninit","1");
    } else {
    ...
    zf_enter("netdev_dpdk_register");
    netdev_dpdk_register();
    zf_out("netdev_dpdk_register","");
    zf_out("dpdk_init__","");
}
