		...
		dpdk_port_t *ret_port_id, int *ret_hot_plugged)
{
	...
	if ((t = snobj_eval(conf, "port_id")) != NULL) {
		if (snobj_type(t) != TYPE_INT){
			zf_out("Find_dpdk_port","Port ID must be an integer");
		...
		if (port_id < 0 || port_id >= RTE_MAX_ETHPORTS){
			zf_out("Find_dpdk_port","Invalid port id");
		...
		if (!rte_eth_devices[port_id].attached){
			...
			zf_out("Find_dpdk_port","Port id %%d is not available");
	...
	if ((t = snobj_eval(conf, "pci")) != NULL) {
		...
		if (port_id != DPDK_PORT_UNKNOWN){
			zf_out("Find_dpdk_port","You cannot specify both 'port_id' and 'pci' fields");
		...
		if (!bdf) {
pci_format_err:
			zf_out("Find_dpdk_port","Pci_format_err");
		...
		if (port_id == DPDK_PORT_UNKNOWN) {
			...
			if (ret < 0){
				zf_out("Find_dpdk_port","Rte_eth_dev_attach failed");
	...
	if (port_id == DPDK_PORT_UNKNOWN &&
		...
		if (ret < 0){
			zf_out("Find_dpdk_port","Cannot attach vdev");
...
static struct snobj *pmd_init_port(struct port *p, struct snobj *conf)
{
	...
	if (err){
    	zf_out("Pmd_init_port","Find_dpdk_port failed");
		return err;
	}
	...
	if (ret != 0){
    	zf_out("Pmd_init_port","Rte_eth_dev_configure() failed");
	...
	for (i = 0; i < num_rxq; i++) {
		...
		if (ret != 0){
    		zf_out("Pmd_init_port","Rte_eth_rx_queue_setup() failed");
	...
	for (i = 0; i < num_txq; i++) {
		...
		ret = rte_eth_tx_queue_setup(port_id, i,
					     ...
					     sid, &eth_txconf);
		if (ret != 0){
    		zf_out("Pmd_init_port","Rte_eth_tx_queue_setup() failed");
			return snobj_err(-ret,
	...
	if (ret != 0){
    	zf_out("Pmd_init_port","Rte_eth_dev_flow_ctrl_get() failed");
...
static void pmd_collect_stats(struct port *p, int reset)
{
	...
	if (reset) {
		...
		zf_out("Pmd_collect_stats","Reset");
		return;
	}
	...
	if (ret < 0) {
		log_err("rte_eth_stats_get() failed: %s\n",
				rte_strerror(-ret));
		zf_out("Pmd_collect_stats","Rte_eth_stats_get failed");
		return;
	}
